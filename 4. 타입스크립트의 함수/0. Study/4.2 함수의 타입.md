...
함수는 값. 함수를 나타내는 타입 존재합니다. `함수라는 값 자체`를 나타내는 타입도 존재 => 그 타입이 함수 타입
...
type 문으로 별명을 붙이거나 타입 표기에 사용할 수 있음

---

```ts
type F2 = (arg:string,arg2: string) => bollean;
// 에러 : type '(num:number) => void' is not assignable to type 'F2'
const fun: F2 (num:number): void => console.log(num);
```

타입에 맞지 않는 함수를 변수에 대입하면 컴파일 에러가 난다.
표기상 변수 fun은 F2타입 이지만 실제로 대입하려는 함수의 타입은 (num:number) =>void라서 대입할수 없음
...
- 명시
  ...
  - 이점(1) : 본문이 긴 경우, 반환값 타입 명시하는 이점이 커짐. 하지만 타입 추론으로 타입스크립트가 추론해서 알려주어서 이점 약해지는 중 (최신에디터 : inlay hints기능)
  - 이점(2) : 함수 내부에서 반환값 타입에 대한 타입 검사를 할 수 있음. => 구현 실수를 미연에 방지 가능
...
_return result;를 빠뜨리는 실수를 하였을때_
...
=> xRepeat의 타입 표기가 없지만 xRepeat에 대입된 식을 참고해서 타입이 (arg: number) => string 이라고 자동으로 판단
